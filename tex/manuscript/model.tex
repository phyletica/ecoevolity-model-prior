...
\citep{Tavare1986}.
Alternatively, if the frequencies are constrained to be equal, the CTMC model
is a two-state analog of the ``JC69'' model of nucleotide substitution
\citep{JC1969}.
For the divergence times among comparisons to be comparable, the relative or
absolute mutation rate (\murate) or each pair must be specified or given a
...
(\murate) or substitution parameters (\gfreq)
...
also need to use trans-dimensional MCMC algorithms to sample over all possbile
divergence models.
